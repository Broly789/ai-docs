...
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span>main();</span></span></code></pre></div><p>根据 query 查询出文档后，放到 prompt 里：</p><p><img src="`+E+'" alt="image-20260128153805941"></p><p>让大模型根据文档回答，并且引用原文片段。</p><p>跑一下：</p><video src="'+b+'" controls></video><p>可以看到，大模型根据片段内容做了回答，并且引用了原文。</p><p>（等很久是因为我们还没做流式输出，其实一直在生成）</p><p>这样，电子书语义检索助手就完成了。</p><p>你也可以把电子书换成公司内部的文档，实现文档检索，流程一样。</p><blockquote><p>代码上传了课程仓库： <a href="https://github.com/QuarkGluonPlasma/ai-agent-course-code" target="_blank" rel="noreferrer">https://github.com/QuarkGluonPlasma/ai-agent-course-code</a></p></blockquote><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">​</a></h2><p>这节我们把 loader、splitter、Milvus 向量数据库串了起来，做了一个电子书语义检索助手的小实战。</p><p>实际上公司项目就是这样来做 RAG 的，只不过 Milvus 存的内容不同，但流程一样。</p><p>现在语义检索出 Milvus 相关记录后，返回了 book_id，这个是对应 MySQL 里 book 表的 id，那是不是就可以顺带着关联查出 book 表和相关表的数据呢？</p><p>这就涉及到了 MySQL 和 Milvus 的联动，我们后面学到 MySQL 部分再继续深入。</p>',87)])])}const w=v(k,[["render",M]]);export{q as __pageData,w as default};
