...
<span class="line"><span></span></span>
<span class="line"><span>console.log(&#39;\\n[最终回复]&#39;);</span></span>
<span class="line"><span>console.log(response.content);</span></span></code></pre></div><p>跑下试试：</p><video src="`+y+'" controls></video><p><img src="'+f+'" alt="image-20260128122409640"></p><p>可以看到，检测到了 tool_calls 工具调用，用 read_file 这个工具读取了文件，然后让大模型分析了文件内容，给出了代码解释。</p><p>是不是现在大模型就能读文件了！</p><p>这就是通过工具给大模型扩展了能力。</p><blockquote><p>代码上传了课程仓库： <a href="https://github.com/QuarkGluonPlasma/ai-agent-course-code" target="_blank" rel="noreferrer">https://github.com/QuarkGluonPlasma/ai-agent-course-code</a></p></blockquote><h2 id="总结" tabindex="-1"><span style="color:rgb(46,161,33);background-color:inherit;">总结</span> <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;&lt;span style=&quot;color: rgb(46,161,33); background-color: inherit&quot;&gt;总结&lt;/span&gt;&quot;">​</a></h2><p>这节我们入门了 langchain，调用了大模型，并且实现了第一个 tool</p><p>我们用的千问的模型，因为它有免费额度，获取 api key 后，用 .env 管理。</p><p>.env 这个文件不提交 git，都是聊天软件发送的方式口口相传，就和数据库密码一样。</p><p>我们用 tool 创建了一个工具，写一下函数，以及加下名字、描述、参数的格式（用 zod 声明）就可以了。</p><p>用 model.bindTools 传给大模型，在 system message 告诉它这个工具的信息，以及规范下它的回答流程。</p><p>message 分为 SystemMessage、HumanMessage、AIMessage、ToolMessage 四种</p><p>之后，直接问大模型某个代码的信息，它就会调用工具读取文件，然后来解答了。</p><p>实现了第一个 tool 之后，你可以想一下 cursor 怎么实现，后面我们实现一个简易版 cursor！</p>',111)])])}const $=x(E,[["render",I]]);export{R as __pageData,$ as default};
